...
export function ESGScorecardArtifact({ data }: Props) {
  ...
  const scoreColor = displayScore == null
    ...
    : displayScore >= 70 ? "from-ok to-ok/80"
    : displayScore >= 40 ? "from-warn to-warn/80"
    ...
    <div className="space-y-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2">
              <span className="text-lg font-bold text-foreground dark:text-foreground">ESG 综合评分</span>
      ...
      <div>
        <div className="text-xs font-medium text-muted-foreground mb-1.5 flex items-center gap-1.5">
          <Leaf className="h-3 w-3 text-ok" /> E/S/G 三维
        </div>
        <div style={{ height: 200 }}>
          <SafeResponsiveContainer width="100%" height="100%">
            <RadarChart data={radarData} margin={{ top: 10, right: 20, left: 20, bottom: 0 }}>
              ...
              <Radar dataKey="score" stroke={palette.chart5} fill={palette.chart5} fillOpacity={0.35} />
            </RadarChart>
          </SafeResponsiveContainer>
        </div>
        <div className="grid grid-cols-3 gap-2 mt-1">
          {[{ k: "E", v: e, cls: "text-ok", icon: Leaf }, { k: "S", v: s, cls: "text-accent", icon: Users2 }, { k: "G", v: g, cls: "text-warn", icon: Scale }].map(it => (
              ...
              <tbody>
                {sources.map((row, i) => (
                  <tr key={i} className="border-t border-foreground/[0.04] dark:border-white/[0.04] hover:bg-foreground/[0.02] dark:hover:bg-white/[0.02]">
                    <td className="px-2.5 py-1.5 text-foreground/90 dark:text-foreground/90">{row.source || "—"}</td>
                    ...
                    <td className="px-2.5 py-1.5 text-center font-mono text-[10px]">
                      <span className="inline-flex items-center px-1.5 py-0.5 rounded bg-accent/10 text-accent">{row.grade || "--"}</span>
                    </td>
      ...
      {climate.length > 0 && (
        <div>
          ...
          <div className="flex flex-wrap gap-1.5">
            {climate.map((c, i) => (
              <span key={i} className="inline-flex items-center gap-1.5 px-2 py-1 rounded-lg text-[10px] bg-ok/10 border border-ok/20 text-ok">
                <span className="font-mono font-medium">{c.tag || "--"}</span>
                {c.label && <span className="text-ok/80">{c.label}</span>}
